...
def generate_robot_launch_actions(context: LaunchContext, *args, **kwargs):
    ...
    sdf_file = os.path.join(
        pkg_project_description, "models", "iris_with_stereo", "model.sdf"
    )
    ...
    # for sdformat_urdf plugin used by robot_state_publisher
    robot_desc = robot_desc.replace(
        "model://stereo_cam", "package://ardupilot_gz_description/models/stereo_cam"
    )

    log = LogInfo(msg="Using iris_with_stereo (Stereo camera, iris_stereo_bridge)")
    config_name = "iris_stereo_bridge.yaml"
