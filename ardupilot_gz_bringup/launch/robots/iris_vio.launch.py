...
"""
Launch an iris quadcopter with stereo cameras for VINS in Gazebo and Rviz.
...
def launch_state_pub_with_bridge(context):
    ...
    # Load SDF file - use stereo camera model for VIO (no gimbal, two static mono cameras)
    sdf_file = os.path.join(
        pkg_ardupilot_gazebo, "models", "iris_with_stereo_camera", "model.sdf"
    )
    ...
    # Bridge - use VIO bridge config for stereo cameras
    bridge = Node(
        ...
        parameters=[
            {
                "config_file": os.path.join(
                    pkg_project_bringup, "config", "iris_vio_bridge.yaml"
                ),
...
def generate_launch_arguments():
    ...
    return [
        ...
        DeclareLaunchArgument(
            "model",
            default_value="iris_with_stereo_camera",
...
def generate_launch_description():
    """Generate a launch description for a iris quadcopter with stereo cameras for VINS."""
